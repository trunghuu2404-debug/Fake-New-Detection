...
from newspaper import Article  # optional, more robust
import joblib
import re
...
def predict():
    ...
    try:
        ...
        # If your model supports predict_proba (e.g., logistic regression, RF, XGB)
        if hasattr(loaded_model, "predict_proba"):
            probs = loaded_model.predict_proba([cleaned_text])[0]
            prob_real, prob_fake = probs[0], probs[1]
        else:
            # Fallback for SVMs without probability=True
        ...
        return jsonify(
            {
                ...
                "probabilities": {
                    "REAL": round(float(prob_real), 3),
                    "FAKE": round(float(prob_fake), 3),
                },
                "article_text_snippet": cleaned_text[:500],
            }
        )
